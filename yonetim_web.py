        ...
        for i, user in enumerate(records):
            if str(user['kullanici_adi']) == str(kadi):
                ...
                if str(user.get('guvenlik_kodu', '')) == str(guvenlik_kodu):
                    # Şifre sütunu (Kullanicilar sayfasında 2. sütun (B) varsayıyoruz)
...
if not st.session_state["giris"]:
    ...
    with c2:
        ...
        if st.session_state["ui_mode"] == "login":
            u = st.text_input("Kullanıcı Kodu", placeholder="Kullanıcı adınız", key="u_log")
            p = st.text_input("Şifre", type="password", placeholder="Şifreniz", key="p_log")
        ...
        st.markdown("<p style='text-align:center; color:#94a3b8; margin-top:30px; font-size:12px;'>Zorlu Soft | © 2026 | v70.1</p>", unsafe_allow_html=True)
    st.stop()
...
st.info("Sistem Excel bağlantısı ile çalışıyor.")
