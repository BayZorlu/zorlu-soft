...
st.set_page_config(
    ...
    layout="wide", 
    page_icon="🏢",
    initial_sidebar_state="collapsed" # Standart menüyü kapalı başlatıyoruz
)
...
# --- CSS: DISCORD TARZI MENÜ TASARIMI ---
st.markdown("""
<style>
    /* 1. STANDART MENÜYÜ VE HEADER'I YOK ET */
    ...
    /* 3. SOL İKON MENÜ STİLİ */
    div[data-testid="column"]:nth-of-type(1) {
        background-color: #2c3e50; /* Koyu Lacivert Arkaplan */
        padding: 10px;
        ...
        left: 0;
        top: 0;
        width: 80px !important; /* Sabit Genişlik */
        z-index: 999;
        overflow-y: auto;
    }
    
    /* ANA İÇERİĞİ SAĞA İT (Menü üstüne binmesin) */
    div[data-testid="column"]:nth-of-type(2) {
        margin-left: 90px; 
    }

    /* BUTON TASARIMLARI (Yuvarlak İkonlar) */
    .stButton button {
    ...
    /* Aktif Buton (Seçili Olan) - Kırmızı Olsun */
...
if "active_menu" not in st.session_state: st.session_state["active_menu"] = "Genel Bakış" # Varsayılan Menü
...
if not st.session_state["giris"]:
    ...
    with c2:
        ...
        st.markdown(f"<div class='login-box'><img src='{aktif_logo}' width='100'><h2>{data['site_adi']}</h2><p>Pro Giriş v34</p></div>", unsafe_allow_html=True)
...
# ==============================================================================
# ANA YAPI (SOL İKON BAR + SAĞ İÇERİK)
# ==============================================================================

# EKRANI İKİYE BÖL (1 BİRİM SOL MENÜ, 15 BİRİM İÇERİK)
col_nav, col_main = st.columns([1, 15])

# --- SOL İKON MENÜ (DISCORD TARZI) ---
with col_nav:
    # Logonun küçüğünü en üste koy
    if os.path.exists(LOGO_DOSYA): st.image(LOGO_DOSYA, use_container_width=True)
    else: st.markdown("🏢")
    
    st.markdown("---")
    
    # YÖNETİCİ MENÜLERİ (İKON EŞLEŞTİRMELERİ)
    if st.session_state["rol"] == "admin":
        ...
        for label, icon in menu_items:
            # Butona basınca session'daki aktif menüyü güncelle
            if st.button(icon, key=f"nav_{label}", help=label, type="secondary"):
...
with col_main:
    # Seçili menüye göre içeriği göster
    ...
    if st.session_state["rol"] == "admin":
        ...
        if menu == "Genel Bakış":
            ...
            if st.button("💾 ZORLA KAYDET"): kaydet(data); st.success("Kaydedildi")
            ...
            st.title("👥 Hesaplar")
            src = st.text_input("🔍 Daire Ara (No veya İsim)")
            filtre = None
            if src:
            ...
            with c2:
                ...
                if pdf_data: st.download_button("📄 Makbuz", pdf_data, f"makbuz_{secilen}.pdf", "application/pdf")
            ...
            st.title("🚗 Otopark"); st.dataframe(pd.DataFrame([{"Plaka":v["plaka"], "Sahip":v["sahip"]} for v in data["daireler"].values() if v["plaka"]!="-"]), use_container_width=True)
    ...
    # SAKİN İÇERİKLERİ
        ...
        if menu == "Durum":
            ...
            st.title("Geçmiş")
            temiz = [x.split("|") if "|" in x else ["-", x] for x in reversed(info["gecmis"])]
            st.table(pd.DataFrame(temiz, columns=["Tarih","İşlem"]))
        elif menu == "Talep":
            st.title("Talep")
