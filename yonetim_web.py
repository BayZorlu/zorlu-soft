...
<style>
    ...
    /* 1. GİZLENECEKLER */
    .stDeployButton, [data-testid="stHeaderActionElements"], [data-testid="stToolbar"], [data-testid="stManageAppButton"], footer, #MainMenu { display: none !important; }
    header[data-testid="stHeader"] { background: transparent !important; height: 0px !important; visibility: hidden !important; }

    /* 2. INPUT DÜZELTME (v70) */
        ...
        for i, user in enumerate(records):
            if str(user['kullanici_adi']) == str(kadi):
                if str(user['guvenlik_kodu']) == str(guvenlik_kodu):
                    # Excel'de şifre sütunu (Genelde 2. sütun olur, tablonuza göre kontrol edin)
                    # Bulunan satır i+2 (Header+0-index düzeltmesi)
                    sheet.update_cell(i + 2, 2, yeni_sifre) 
                    return True, "Şifreniz Excel'de güncellendi. Giriş yapabilirsiniz."
                else:
                    return False, "Güvenlik kodu eşleşmedi!"
...
if not st.session_state["giris"]:
    ...
    with c2:
        ...
        if st.session_state["ui_mode"] == "login":
            u = st.text_input("Kullanıcı Kodu", placeholder="Excel'deki kullanıcı adınız", key="log_u")
            p = st.text_input("Şifre", type="password", placeholder="Şifreniz", key="log_p")
            ...
            if st.button("GİRİŞ YAP", type="primary", use_container_width=True):
                ...
                if user_data:
                    ...
                    st.rerun()
                else: st.error("Giriş bilgileri Excel ile eşleşmedi.")
...
st.write("Veriler Excel (Google Sheets) üzerinden canlı olarak çekilmektedir.")
